...
void primary_index_batch_delete(dpu_set_context *set, PRIMARY_INDEX_ID id, char **keys, uint32_t key_len, uint32_t batch_size)
{
    int64_t begin, last, cur;
    pthread_mutex_lock(&(set->lock));
    dpu_timer_reset(&(set->timer));
    ...
    last = begin;
    cur = begin;
    dpu_push_build_package_primary_index_delete(&(set->push_info), id, keys, key_len, batch_size);
    last = cur;
    cur = usec();
    set->timer.load_balance_interval += cur - last;
    dpu_push_package(&(set->push_info), set->interface);
...
void primary_index_batch_delete_v1(PRIMARY_INDEX_ID id, char **keys, uint32_t key_len, uint32_t batch_size)
{
    dpu_set_context *dpu_set;
    dpu_set = dpu_index_manager_get_dpu_set(id);
    if (dpu_set)
    {
        primary_index_batch_delete(dpu_set, id, keys, key_len, batch_size);
    }
}
