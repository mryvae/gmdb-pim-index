...
void op_primary_index_delete_prepare()
{
    ...
    for (int j = 0; j < global_num_pre_load_primary_index; j++)
    {
        if (primary_index_id == global_prestored_primary_index[j].index_id)
        {
            ...
            global_minus = (coo_matrix_v_dpu)global_coo_A;
            global_minus_nnz = global_plus->nnz;
            break;
        }
    }
}
...
void op_primary_index_delete_tasklets_run()
{
    ...
    {
        minus_start = BLOCK_LOW(tasklet_id - 1, NR_SLAVE_TASKLETS, global_minus_nnz);
        minus_nnz = BLOCK_SIZE(tasklet_id - 1, NR_SLAVE_TASKLETS, global_minus_nnz);
        ...
        if (minus_nnz > 0)
        {
            start = (__mram_ptr coo_matrix_v_elem *)(&(global_minus->data[minus_start]));
        ...
        handle_elem:
            ...
            {
                primary_index_dpu_delete(global_primary_index, current_elem->row, &global_index_block_mram_allocator);
            }
        }
